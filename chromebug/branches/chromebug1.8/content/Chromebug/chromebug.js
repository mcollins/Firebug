...
const fbBox = document.getElementById("fbContentBox");
const interfaceList = document.getElementById("cbInterfaceList");
const inspectClearProfileBar = document.getElementById("fbToolbar");
const appcontent = document.getElementById("appcontent");
const cbContextList = document.getElementById('cbContextList');
const cbPackageList = document.getElementById('cbPackageList');
...
Firebug.Chromebug = Obj.extend(Firebug.Module,
{
    ...
    onXULWindowAdded: function(xul_window, outerDOMWindow)
    {
        try
        {
            ...
            context.onLoad = context.loadHandler.bind(context);
            outerDOMWindow.addEventListener("DOMContentLoaded", context.onLoad, true);

            context.onUnload = context.unloadHandler.bind(context)
            outerDOMWindow.addEventListener("unload", context.onUnload, true);

            outerDOMWindow.addEventListener("keypress", Chromebug.XULAppModule.keypressToBreakIntoWindow.bind(this, context), true);
        }
        catch(exc)
        {
    ...
    createBrowser: function(global, fileName)
    {
        ...
        else if (Url.isDataURL(browserName))
        {
        ...
        browser.currentURI = Url.makeURI(browserName);
    ...
    getPlatformStringURL: function(string)
    {
        ...
        {
            FBTrace.sysout("getPlatformStringURL FAILS for "+string, exc);
        }
    },
    ...
    initialize: function()
    {
       ...
       // this.setLocators(document);
        ...
        this.retryRestoreID = setInterval(Firebug.Chromebug.restoreState.bind(this), 500);
        ...
        {
            FBTrace.sysout("unloading " + window.location, event);
        }, true);
    },
    ...
    prepareForCloseEvents: function()
    {
        ...
        this.onUnloadTopWindow = this.onUnloadTopWindow.bind(this); // set onUnload hook on the domWindow of our chromebug
    ...
    toggleIntroduction: function()
    {
        ...
        {
            document.getElementById('content').removeAttribute("collapsed");
            fbBox.setAttribute("collapsed", true);
        }
        else
        {
            document.getElementById('content').setAttribute("collapsed", true);
...
Chromebug.SourceFileListBase.prototype = Obj.extend(new Firebug.Listener(),
{
    ...
    {
        var getDescription = this.getDescription.bind(this);
...
Chromebug.connectedList = function(xul_element, list)
{
    if (!list.elementBoundTo)
    {
        ...
        xul_element.addEventListener("selectObject", list.onSelectLocation.bind(list), false);
        if (list.onPopUpShown)
            xul_element.addEventListener("popupshown", list.onPopUpShown.bind(list), false);
    }
    return list;
}
...
Firebug.Chromebug.PackageList = Obj.extend(new Firebug.Listener(),
{
...
Firebug.Chromebug.contextList =
{
    ...
    {
        FBTrace.sysout('getObectDescription '+context.getName());
        try
        {
        ...
        if (FBTrace.DBG_LOCATIONS)
            ...
            if (FBTrace.DBG_LOCATIONS)
                FBTrace.sysout("getObjectDescription for context "+context.getName()+" Fail "+exc, exc)
        }
        return d;
    },
...
Firebug.Chromebug.allFilesList = Obj.extend(new Chromebug.SourceFileListBase(), {
    ...
    onPanelNavigate: function(object, panel)
    {
        ...
        if (sourceFile)
            document.getElementById('cbAllFilesList').location = this.getDescription(sourceFile);
    },
