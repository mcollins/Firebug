...
function createFirebugConsole(context, win)
{
    ...
    function logAssert(category, args)
    {
        ...
        if (msg.stack)
        {
            ...
            if (FBTrace.DBG_CONSOLE)
                FBTrace.sysout("consoleExposed logAssert trace from msg.stack", trace);
        }
        else if (context.stackTrace)
        {
            var trace = context.stackTrace
            if (FBTrace.DBG_CONSOLE)
                FBTrace.sysout("consoleExposed logAssert trace from context.window.stackTrace", trace);
        }
        else
        {
            var trace = getJSDUserStack();
            if (FBTrace.DBG_CONSOLE)
                FBTrace.sysout("consoleExposed logAssert trace from getJSDUserStack", trace);
        }
    ...
    function getComponentsStackDump()
    {
        ...
        if (FBTrace.DBG_CONSOLE)
            FBTrace.sysout("consoleExposed consoleInjector.getComponentsStackDump initial stack for userURL "+userURL, frame);
        ...
        if (FBTrace.DBG_CONSOLE)
            FBTrace.sysout("consoleExposed consoleInjector.getComponentsStackDump final stack for userURL "+userURL, frame);
    ...
    function getJSDUserStack()
    {
        ...
        if (frames && (frames.length > 0) )
        {
            ...
            if (FBTrace.DBG_CONSOLE)
                FBTrace.sysout("consoleExposed consoleInjector getJSDUserStack: "+frames.length+" oldest: "+
