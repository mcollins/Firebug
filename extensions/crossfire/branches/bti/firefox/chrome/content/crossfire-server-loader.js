...
function getModuleLoaderConfig(baseConfig)
{
    ...
    baseConfig.arch = baseConfig.arch ||  "remoteServer";
    baseConfig.baseUrl = baseConfig.baseUrl || "resource://";
    baseConfig.paths = baseConfig.paths || {"arch": "crossfireModules/tools/server", "firebug": "firebug_rjs", "crossfireModules":"crossfireModules"};
...
function(CrossfireModule, CrossfireUI, CrossfirePanel, ChromeFactory, FBL, Firebug)
{
    try
    {
        ...
        {
            FBTrace.sysout("Crossfire server loader ; legacyPatch");
    ...
    {
        if (FBTrace)
            FBTrace.sysout("Crossfire server loader initialization ERROR "+exc, exc);
        window.dump("Crossfire server loader initialization ERROR "+exc+"\n");
        if (Components)
