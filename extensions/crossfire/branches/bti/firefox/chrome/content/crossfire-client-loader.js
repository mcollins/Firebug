...
function getModuleLoaderConfig(baseConfig)
{
    ...
    baseConfig.arch = baseConfig.arch ||  "remoteClient";
    baseConfig.baseUrl = baseConfig.baseUrl || "resource://";
    baseConfig.paths = baseConfig.paths || {"arch": "crossfireModules/tools/client", "firebug": "firebug_rjs", "crossfireModules":"crossfireModules"};
...
/*
if (FBTrace && FBTrace.DBG_INITIALIZE || FBTrace.DBG_MODULES)
{
...
function(CrossfireModule, CrossfireUI, ChromeFactory, FBL, Firebug)
{
    try
    {
        ...
        window.top.CrossfireUI = CrossfireUI;

        /*if (something.RemoteTool) {
        ...
        */
        ...
        {
            FBTrace.sysout("Crossfire client loader ; legacyPatch");
    ...
    {
        if (FBTrace)
            FBTrace.sysout("Crossfire client loader initialization ERROR "+exc, exc);
        window.dump("Crossfire client loader initialization ERROR "+exc+"\n");
        if (Components)
