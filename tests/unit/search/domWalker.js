function calcExpectedContent() {
    ...
    for (var expectedOffset = 0; expectedOffset < expectedContent.length; expectedOffset++)
    {
        if (!expectedContent[expectedOffset]) {
            ...
            fireunit.ok(doc, "Document " + iframeOffset + " exists");
    ...
    fireunit.compare(iframeOffset, iframes.length, "All iframes used.");
...
function domWalkerBoundaryTest() {
    ...
    fireunit.ok(!walker.nextNode(), "Past End Condition Holds");
    fireunit.ok(walker.previousNode(), "Past End -> End Move Succeeds");
    fireunit.ok(!walker.nextNode(), "Past End Condition Holds");
    
    walker.reset();
    while (walker.previousNode());
    fireunit.ok(!walker.previousNode(), "Past Start Condition Holds");
    fireunit.ok(walker.nextNode(), "Past Start -> Start Move Succeeds");
    fireunit.ok(!walker.previousNode(), "Past Start Condition Holds");
}
function domWalkerTest(expected, stepSize, reverse) {
    ...
    function verifyNode(node, index) {
        ...
        if (!node || !expectedNode) {
            fireunit.compare(expectedNode, node, "existence: " + index);
            verifyFail = true;
        }
        if (!verifyFail && expectedNode.type != node.nodeType) {
            fireunit.compare(expectedNode.type, node.nodeType, "nodeType: " + index + " " + node);
            verifyFail = true;
        }
    ...
    fireunit.ok(!failed, "domWalkerTest: " + stepSize + " " + reverse + " " + expected.length);
}
